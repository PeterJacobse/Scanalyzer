...
def get_scan(file_path, units: dict = {"length": "m", "current": "A"}, default_channel_units: dict = {"X": "m", "Y": "m", "Z": "m", "Current": "A", "LI Demod 1 X": "A", "LI Demod 1 Y": "A", "LI Demod 2 X": "A", "LI Demod 2 Y": "A"}):
    if not os.path.exists(file_path):
        print(f"Error: File \"{file_path}\" does not exist.")
        return
    root, ext = os.path.splitext(file_path)
    if ext != ".sxm":
        print(f"Error: File \"{file_path}\" is not an sxm file.")
        return
    
    scan_object = nap.read.Scan(file_path) # Read the scan data. scan_data is an object whose attributes contain all the data of the scan
    channels = np.array(list(scan_object.signals.keys())) # Read the channels
    scan = scan_object.signals
    header = scan_object.header
    up_or_down = header.get("scan_dir", "down") # Read whether the scan was recorded in the upward or downward direction
    pixels_uncropped = header.get("scan_pixels", np.array([100, 100], dtype = int)) # Read the number of pixels in the scan
    scan_range_uncropped = header.get("scan_range", np.array([1E-8, 1E-8], dtype = float)) # Read the size of the scan
    bias = round(float(header.get("bias", 0)), 3) # Get the bias (present in the header as a string, passed more directly as a float)
    z_controller = header.get("z-controller") # Extract and convert z-controller parameters
    feedback = bool(z_controller.get("on", True)[0]) # Bool, true or false
    setpoint_str = z_controller.get("Setpoint", "1E-11")[0]
    
    # Extract and convert time parameters and convert to datetime object
    rec_date = [int(element) for element in header.get("rec_date", "00.00.1900").split(".")]
    rec_time = [int(element) for element in header.get("rec_time", "00:00:00").split(":")]
    ...
    for channel in channels:
        for direction in ["forward", "backward"]:
            if channel in length_channels: scan[channel][direction] = np.array(scan[channel][direction] * L_multiplication_factor, dtype = float)
            elif channel in current_channels: scan[channel][direction] = np.array(scan[channel][direction] * I_multiplication_factor, dtype = float)
    
    # Stack the forward and backward scans for each channel in a tensor. Flip the backward scan
    scan_tensor_uncropped = np.stack([np.stack((np.array(scan[channel]["forward"], dtype = float), np.flip(np.array(scan[channel]["backward"], dtype = float), axis = 1))) for channel in channels])
    ...
    setattr(scan_object, "bias", bias)
    setattr(scan_object, "channels", channels)
    setattr(scan_object, "scan_tensor_uncropped", scan_tensor_uncropped) # Uncropped means the size of the scan before deleting the rows that were not recorded
    setattr(scan_object, "pixels_uncropped", pixels_uncropped)
    setattr(scan_object, "scan_range_uncropped", scan_range_uncropped)
    setattr(scan_object, "scan_tensor", scan_tensor)
    setattr(scan_object, "pixels", pixels)
    setattr(scan_object, "scan_range", scan_range)
    setattr(scan_object, "feedback", feedback)
    setattr(scan_object, "setpoint", setpoint)
